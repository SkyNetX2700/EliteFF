...
import { Trophy, RefreshCw, ChevronRight, Users, Skull, History, Search, X, BookOpen, CheckCircle, Clock, AlertCircle, Lock } from "lucide-react";
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {t.isPrivate && (
                <>
                  <span className="text-xs" style={{ color: "var(--th-dimmer)" }}>·</span>
                  <span className="text-xs font-bold flex items-center gap-1" style={{ color: "#fbbf24" }}><Lock size={10} /> Private</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 flex-wrap mb-3">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold text-foreground">
              {formatDateTime12(t.scheduledAt, false)}
            </span>
          </div>
          <div className="flex items-center gap-1.5">
...
export default function Tournaments() {
  ...
  const filtered = tabFiltered.filter((t: any) => {
    ...
    // Players only see completed tournaments they joined; hosts need their full history.
    if (!user?.isHost && tab === "all" && t.status === "completed" && !myTournamentIds.has(t.id)) return false;
